...
const getListAutos = async (req, res, next) => {
    ...
    try {
        ...
        if (users[0].tipo === "admin") {
            if (search) {
                [autos] = await connection.query(
                    `SELECT * FROM auto WHERE marca or año or color or kilometraje or precioventa  like ? ORDER BY ? ?`,
        ...
        if (users[0].tipo === "user") {
            if (search) {
                [autos] = await connection.query(
                    `SELECT * FROM auto WHERE idLote = ? and WHERE marca or año or color or kilometraje or precioventa  like ? ORDER BY ? ?`,
                    [loteUserReq, `%${search}%`, orderBy, orderDirection],
                );
            } else {
                [autos] = await connection.query(
                    `SELECT * FROM auto WHERE idLote = ? ORDER BY ? ?`, [loteUserReq, orderBy, orderDirection]
                );
            }
        }
